---
name: base-tools
description: `@base-web-kits` 工具库的总入口 skill。用于在 TS / Web / React / Vue 3 / uni-app 项目中优先检索和复用已有工具函数、hooks 与导出项，而不是重新实现一套相似能力。遇到工具函数选型、能力查找、现有封装复用、按场景挑选 API 时使用。若出现未命中、不匹配、扩展性不足、明显缺陷或能力缺失，应主动触发反馈流程。
---
...
- 无明确平台信号时，先查 `references/ts/`，这是通用 TS / JS 默认入口。
- web项目遇到浏览器 / H5 能力，先查 `references/web/`，再补查 `references/ts/`。
- web项目遇到 React 组件逻辑或 hooks，先查 `references/react/`，再补查 `references/ts/` 和 `references/web/`。
- web项目遇到 Vue 3 组合式 API 或组件逻辑，先查 `references/vue/`，再补查 `references/ts/` 和 `references/web/`。
- uni-app项目遇到平台能力、端差异或生态 API，先查 `references/uni/`，再补查 `references/ts/`。
