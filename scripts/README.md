...
- 另外, 工具库本身很多地方也用到这些第三方库, 比如 es-toolkit 中的函数
- 为了解决vue2+webpack不支持es6模块的问题, 会对这些第三方库进行打包, 并转译为 es2015 版本 (在'scripts/build/tsup.xx.ts'中配置noExternal和target)
- 为了提供类型提示, 会在 npm 包 package.json 的 `dependencies` 中声明这些第三方库的版本, 仅提供ts类型资源
...
- web和uni目录如果使用ts目录的re-export函数, 应直接引用, 如 `import { pickBy } from 'es-toolkit'`, noExternal也要配置, 因为vue2+webpack默认不编译node_modules的es6
