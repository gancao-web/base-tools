import { getAppConfig, promisifyUniApi, downloadFile } from '../index';

/**
 * 拼接完整的文件地址
 * - 需在入口文件初始化应用配置 setAppConfig({ hostFile })
 * @param src 完整/相对文件地址
 */
...
/**
 * 拼接完整的图标地址
 * - 需在入口文件初始化应用配置 setAppConfig({ hostIcon })
 * @param src 相对图标地址
 */
export function getIconUrl(icon: string) {
  const { hostIcon } = getAppConfig();
